...
class FundWidget : GlanceAppWidget() {
    override suspend fun provideGlance(context: Context, id: GlanceId) {
        val db = Room.databaseBuilder(context, AppDatabase::class.java, "fund_helper.db").build()
...
private fun FundWidgetContent(funds: List<FundEntity>) {
    GlanceTheme {
        Column(
            ...
            if (funds.isEmpty()) {
                Text("暂无自选基金", style = TextStyle(fontSize = 12.sp, color = GlanceTheme.colors.onSurfaceVariant))
            } else {
                LazyColumn {
                    items(funds) { fund ->
                        Row(
                            ...
                            Text(fund.name, style = TextStyle(fontSize = 12.sp), modifier = GlanceModifier.defaultWeight())
                            Text(fund.code, style = TextStyle(fontSize = 11.sp, color = GlanceTheme.colors.onSurfaceVariant))
                        }
                    }
                }
            }
            Spacer(modifier = GlanceModifier.height(4.dp))
            Button(text = "刷新", onClick = actionRunCallback<RefreshAction>())
        }
    }
}
